...
class InteLocator : public rclcpp::Node {
public:
    InteLocator() : Node("inte_locator") {
        subscriber_ = this->create_subscription<sensor_msgs::msg::Image>(
            "image", 10, std::bind(&InteLocator::recev_callback, this, std::placeholders::_1));
        publisher_ = this->create_publisher<std_msgs::msg::Int32MultiArray>("pixel_coordinate", 10);
        model_init();
    }
...
private:
    void recev_callback(sensor_msgs::msg::Image::SharedPtr msg) {
        ...
        for (int idx : indices) {
            ...
            double distance = measure_distance(measurePoints);

            //计算并发送像素坐标
            cv::Point2d pixelCoordinate(
                (boxes[idx].tl().x + boxes[idx].br().x) / 2, (boxes[idx].tl().y + boxes[idx].br().y) / 2);
            RCLCPP_INFO(this->get_logger(), "Pixel coordinate: (%f, %f)", pixelCoordinate.x, pixelCoordinate.y);
            std_msgs::msg::Int32MultiArray msg;
            msg.data.push_back(pixelCoordinate.x);
            msg.data.push_back(pixelCoordinate.y);
    ...
    double measure_distance(const std::vector<cv::Point2f>& imagePoints){
        cv::Mat rvec, tvec;
...
int main(int argc, char* argv[]) {
    ...
    rclcpp::spin(std::make_shared<InteLocator>());
    rclcpp::shutdown();
    return 0;
}