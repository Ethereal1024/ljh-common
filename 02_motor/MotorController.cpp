...
void set_position() {  // 设置电机相位
    ...
    cout << "请输入旋转相位(单位: °):\n";
    cin >> position;
    if (my_motor.set_position(position))
        cout << "电机相位已成功旋转到: " << fixed << setprecision(2) << position << "°.\n";
    else
        cout << "电机处于运行状态，请先关闭电机.";
    _pause();
}
...
void set_speed() {  // 修改电机速度
    ...
    cout << "请输入设置速率(单位: °/ms):\n";
    cin >> speed;
    if (my_motor.set_speed(speed))
        cout << "电机速度已成功设置为: " << fixed << setprecision(2) << speed << "°/ms.\n";
    else
        cout << "电机处于关闭状态, 请先开启电机.";
    _pause();
}
...
void set_acc() {  // 修改电机加速度
    ...
    cout << "请输入设置加速度(单位: °/ms²):\n";
    cin >> acc;
    if (my_motor.set_acc(acc))
        cout << "电机加速度已成功设置为: " << fixed << setprecision(2) << acc << "°/ms².\n";
    else
        cout << "电机处于关闭状态，请先开启电机.";
    _pause();
}
